            ...
            <div className="chart-container">
                <div className="legend">
                    <div>
                        ...
                        <button className={!showHome && 'strike'} onClick={ () => this.setState({ home: !this.state.home}) }><div id="circle"></div><p>{home.teamName}</p></button>
                    </div>
                    <div>
                        <button className={ !goals && 'strike'  } onClick={ () => this.setState({ goals:   !this.state.goals})   }><p className='goals'>Goals</p></button>
                        <button className={ !shots && 'strike'  } onClick={ () => this.setState({ shots:   !this.state.shots})   }><p className='shots'>Shots</p></button>
                        <button className={ !missed && 'strike' } onClick={ () => this.setState({ missed:  !this.state.missed})  }><p className='missed'>Missed Shots</p></button>
                        <button className={ !blocked && 'strike'} onClick={ () => this.setState({ blocked: !this.state.blocked}) }><p className='blocked'>Blocked Shots</p></button>
                        <button className={ !hits && 'strike'   } onClick={ () => this.setState({ hits:    !this.state.hits})    }><p className='hits'>Hits</p></button>
                    </div>
                </div>
